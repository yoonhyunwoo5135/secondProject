	...
	<div id="middle">
		<div id="album">
			...
			<%
				...
				Elements mtitle = element.select("div.title > span.txt.ellipsis");
				Elements img = element.select("div.main-visual > div.cover > img");//메인 이미지
			%>
			<hr>
			<p style="font-size: 20px; font-style: italic; text-align: center;"><%=mtitle.text()%></p>
			<hr>
			<%=img.toString()%>
			<hr>
			<%
				for (Element ptitle : element.select("div.magazine-view > h3.hd")) {
					cnt++;
					for (Element content : element.select("div.view-template.view-" + cnt + "> div.txt"))
			%>
